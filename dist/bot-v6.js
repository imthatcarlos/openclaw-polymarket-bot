...
async function onTick(price) {
    ...
    try {
        ...
        const signal = generateSignal(windowOpenPrice, price, market.upPrice, market.downPrice, timeInWindow, state.config, { up: market.upBestAsk, down: market.downBestAsk });
        lastSignal = signal;
        if (!signal.direction) {
            state.skips++;
            checking = false;
