...
export function generateSignal(closes, windowOpenPrice, currentPrice, marketUpPrice, marketDownPrice, config = DEFAULT_CONFIG) {
    ...
    for (let i = 20; i < closes.length; i++)
        ema20 = (closes[i] - ema20) * 2 / 21 + ema20;
    ...
    if (!direction) {
        ...
        if (kExhaustedLow)
            reasons.push("⚠️ Exhaustion: K pinned low but momentum fading");
        if (kExhaustedHigh)
            reasons.push("⚠️ Exhaustion: K pinned high but momentum fading");
        // ── Deceleration veto ──
        ...
        if (lastK < config.oversoldThreshold && lastK > lastD) {
            if (kExhaustedLow) {
                ...
                if (lastRSI < 30) {
                    confidence += 0.1;
                    reasons.push("RSI confirms oversold");
                }
                if (momAccel > 0.01) {
                    confidence += 0.1;
            ...
            if (kExhaustedHigh) {
                ...
                if (lastRSI > 70) {
                    confidence += 0.1;
                    reasons.push("RSI confirms overbought");
                }
                if (momAccel < -0.01) {
                    confidence += 0.1;
                    reasons.push("Momentum accelerating down");
                }
            }
            else {
